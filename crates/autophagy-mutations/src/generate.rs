...
/// Exclusion text stamped into every failure-recovery candidate generated by
/// this crate. It must stay true at every lifecycle stage a package can reach
/// — including after it has been installed, long past replay and shadow
/// evaluation — because the exclusion is part of the immutable package and is
/// never rewritten once registered (see `crates/autophagy-install`, which
/// renders it verbatim into the installed `SKILL.md`).
pub const ADVISORY_EXCLUSION: &str = "Do not block execution; this instruction is advisory.";

/// The exclusion text this crate generated before [`ADVISORY_EXCLUSION`] was
/// introduced. Already-registered mutation packages are immutable and audit-
/// logged, so this historical phrasing can still appear in stored packages;
/// exported so `crates/autophagy-install` can recognize it structurally (exact
/// match, not fuzzy) and avoid rendering the now-false "advisory until replay
/// and shadow evaluation pass" claim for packages installed after those stages
/// already passed.
pub const LEGACY_ADVISORY_UNTIL_REPLAY_EXCLUSION: &str =
...
fn failure_candidate(finding: &EvidencePacket) -> Option<MutationPackage> {
    ...
    Some(base_package(
        ...
        vec![
            "The command can fail transiently even when all preconditions are satisfied."
                .to_owned(),
