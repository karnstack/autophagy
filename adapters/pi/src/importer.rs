...
use crate::{
    ADAPTER_NAME,
    discovery::{DiscoveryPlan, PiDiscoveryError, discover},
    normalize::{FileState, NormalizeContext, normalize_record},
};

/// Controls for one Pi session import.
#[allow(clippy::struct_excessive_bools)]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PiImportOptions {
    /// Pi sessions directory, or one explicit session file.
    pub input: PathBuf,
    /// Stable identity for this Pi installation.
...
/// One bounded, source-addressed Pi diagnostic.
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
pub struct PiImportDiagnostic {
...
/// Aggregate result and exact discovery plan for a Pi import.
#[derive(Clone, Debug, Eq, PartialEq, Serialize)]
pub struct PiImportSummary {
    ...
    pub diagnostics: Vec<PiImportDiagnostic>,
...
/// Discover, normalize, and incrementally import Pi session transcripts.
///
/// # Errors
...
pub fn import_pi(
    ...
    let mut summary = PiImportSummary {
    ...
    for discovered in &discovery.files {
        ...
        if !options.dry_run {
            if let Some(cursor) = store
                .as_deref()
                .ok_or(PiImportError::MissingStore)?
        ...
        loop {
            ...
            offset += u64::try_from(bytes).map_err(|_| PiImportError::PositionOverflow)?;
            line_number += 1;
            ...
            summary.events_emitted +=
                u64::try_from(outcome.events.len()).map_err(|_| PiImportError::PositionOverflow)?;
...
fn validate_options(options: &PiImportOptions) -> Result<(), PiImportError> {
    if options.instance_key.trim().is_empty() {
        return Err(PiImportError::InvalidOptions(
...
fn prefix_hash(path: &Path, consumed: u64) -> Result<[u8; 32], PiImportError> {
    let mut file = File::open(path)?;
    let limit = consumed.min(4096);
    let mut bytes = vec![0; usize::try_from(limit).map_err(|_| PiImportError::PositionOverflow)?];
...
fn search_projection(event: &Event, options: &PiImportOptions) -> SearchProjection {
...
fn reject(
    ...
    if summary.diagnostics.len() < options.max_diagnostics {
        summary.diagnostics.push(PiImportDiagnostic {
            file: file.to_owned(),
            line,
            code: code.to_owned(),
            message,
...
/// Fatal Pi adapter failure.
#[derive(Debug, thiserror::Error)]
pub enum PiImportError {
    /// Option validation failed.
    #[error("invalid Pi import option: {0}")]
    InvalidOptions(String),
    /// Metadata-only discovery failed.
    #[error(transparent)]
    Discovery(#[from] PiDiscoveryError),
    /// Session I/O failed.
    #[error("could not read Pi session: {0}")]
    ...
    #[error("could not serialize Pi cursor state: {0}")]
    ...
    #[error("Pi source position exceeds supported integer range")]
    PositionOverflow,
}
